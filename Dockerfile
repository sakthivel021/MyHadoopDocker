...
RUN apk add --no-cache openssh libressl openssl openjdk8-jre rsync bash procps
# set JAVA_HOME
...
# install Hadoop
RUN wget -O hadoop.tar.gz  https://archive.apache.org/dist/hadoop/core/hadoop-2.7.4/hadoop-2.7.4.tar.gz && \
...
CMD ln -s /usr/local/hadoop-2.7.4 /usr/local/hadoop
